...
from calibrate_fk.utils import overlay_images, read_data, check_model_path, check_data_path, read_joint_names

argument_parser = argparse.ArgumentParser()
argument_parser.add_argument("--model", "-m", help="calibrated robot model. Saved in the calibrated_urdf folder. ")
argument_parser.add_argument("--data", "-d", type=str, nargs='+',
                             help="Data that you want to train the calibration on. Stored in data folder.")
argument_parser.add_argument("--camera-settings", "-c", help="Camera settings file", default="camera_settings.json")
argument_parser.add_argument("--no-generate-images", "-n",
                             help="Pass this flag if you do not want to generate images again but only perform the overlay",
                             action="store_true")
...
def show_configurations():
    ...
    for keys, joints in data.items():
        ...
        q_index = 0
        for a_j in actuated_joints:
            idx = robot.actuated_joints.index(a_j)
            q_show[idx] = q_0[q_index]
            q_index += 1
...
def generate_overlay():
    ...
    for keys, joints in data.items():
        ...
        robot.update_cfg(q_show)
        print(
            "Move the view such that you can nicely see the end-effector. Press q in the visualization window to save the camera settings.")
        robot.show()
        saved_camera = {
...
#     if joint.type == "revolute" and joint in actuated_joints:
...
if generate_images:
    ...
    joint_names = read_joint_names(data_path[0])

    actuated_joints = robot.actuated_joints
    if joint_names:
        actuated_joints = [joint for joint in robot.actuated_joints if joint.name in joint_names]
    dof = len(actuated_joints)

    # generate overlay or switch through configurations
    print("Do you want to generate images for the overlay [0] or observe the configurations [1]? ")
